...
def common_verificacao_api_token(api_token: int):
    if api_token != API_TOKEN:
        logger.error(f"Token INVÁLIDO: {api_token}")
...
def soma(numero1: int, numero2: int, api_token: int):
    ...
    if total < 0:
        ...
        raise HTTPException(status_code=400, detail="O resultado da soma não pode ser negativo")
   
    logger.info(f"Requisição processada com SUCESSO!!!  Resultado da soma: {total}")
   
    return {"resultado": total, "WARNING": "Esta versão de Endpoint será descontinuada em 30 dias"}
...
def operacao_matematica(numeros: Numero, operacao: TipoOperacao):
